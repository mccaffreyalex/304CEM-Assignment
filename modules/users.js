...
exports.validateUser = function validateUser(req, res, next) {
    ...
    const data = req.body
    console.log('alex')
    if (!data.username || !data.password) return res.send(400, {
...
exports.validateCode = function validateCode(req, res, next) {
    ...
    if (!req.body.confirmation) return res.send(400, {
        message: 'Must supply a confirmation code'
    })
    return next()
}
exports.add = function add(req, res, next) {
    ...
    usersConnection(res, usersDB => {
        ...
        if (usersDB.getItemSync(user.username)) return res.send(400, {
        ...
        return res.send(201, {
            message: 'User added, please confirm registration using code'
    ...
    usersConnection(res, usersDB => {
        usersDB.getItem(username, (err, user) => {
            ...
            if (user && user.code === code) {
                ...
                return res.send(400, {
                    message: 'user does not exist or code does not match'
                })
            }
        })
    })
...
exports.delete = function delete_(req, res, next) {
    ...
    if (username !== authuser) return res.send(400, {
    ...
    usersConnection(res, usersDB => {
        usersDB.removeItem(username, err => {
            ...
            return res.send({
                message: `User ${username} and all favourites deleted`
            })
        })
    })
}