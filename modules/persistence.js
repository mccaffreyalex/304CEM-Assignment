...
exports.addUser = (username, password) => new Promise((resolve, reject) => {
    ...
    userSchema.password = userSchema.generateHash(password)
    userSchema.save(function (err, user) {
        err ? reject(new Error('User not saved' + err)) : resolve(user)
    })
})

exports.getUser = (username, password) => new Promise((resolve, reject) => {
    if (username === undefined || password === undefined) reject(new Error('Username/password missing'))
    schema.userModel.findOne({
        username: username
    }, function (err, user, pwd) {
        (err || !user || !user.validPassword(password)) ? reject({message: 'Incorrect password/user does not exist'}) : resolve(user)
    })
})

exports.updateUser = (username, password) => new Promise((resolve, reject) => {
    if (username === undefined || password === user.validPassword(password)) reject(new Error('Username/password missing'))
    schema.userModel.findOneAndUpdate({
        ...
        (err || !user) ? reject({message: 'Incorrect password/user does not exist'}) : resolve(user)
    })
})
